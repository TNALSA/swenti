...
def check_duplicate_article(title, publisher):
    try:
        conn = con_db()
        cur = conn.cursor()
        cur.execute("SELECT COUNT(*) FROM article WHERE title = %s AND publisher = %s", (title, publisher))
...
def insert_article(title, publisher, link, image, site):
    try:
        ...
        cur.execute("INSERT INTO article (title, publisher, link, image, site ) VALUES (%s, %s, %s, %s, %s)", (title, publisher, link, image, site))
        # 반영한 데이터 commit
        conn.commit()
    # db connection error 발생 시
    except Exception as err:
